...
`;
    ...
    if(isBFECC == false){
        ...
        vec2 spot_new3_pos = spot_new3;
        vec2 spot_old2 = spot_new3_pos - vel_2 * dt * ratio;
...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props: any) {
        const dist = Mouse.coords.distanceTo(Mouse.coords_old);
        const steps = Math.max(1, Math.min(30, Math.floor(dist * 200))); // Dynamic steps based on speed

        const forceX = (Mouse.diff.x / 2) * props.mouse_force / steps;
        const forceY = (Mouse.diff.y / 2) * props.mouse_force / steps;
        ...
        if (this.mouse) {
          ...
          for (let i = 0; i < steps; i++) {
            const t = (i + 1) / steps;
            const curX = Mouse.coords_old.x + (Mouse.coords.x - Mouse.coords_old.x) * t;
            const curY = Mouse.coords_old.y + (Mouse.coords.y - Mouse.coords_old.y) * t;

            const centerX = Math.min(
              Math.max(curX, -1 + cursorSizeX + props.cellScale.x * 2),
              1 - cursorSizeX - props.cellScale.x * 2
            );
            const centerY = Math.min(
              Math.max(curY, -1 + cursorSizeY + props.cellScale.y * 2),
